...
We iterate on the given paragrah line by line; and for each line, we iterate on
every two characters to see whether it identifies as a start or closing of the
comment. if so, we ignore; else, we can append the current section onto our
result.
